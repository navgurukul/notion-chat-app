...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- Layer 2: Route the query ---
    const parsed = parseQuery(trimmedMessage);

    // Property-based queries: answer directly from SQL, no AI needed
    if (parsed.kind !== "semantic") {
      ...
      // If metadata handler returns null (couldn't extract target), fall through to semantic
    }

    // Semantic queries: use clean message for DB search, contextual query only for LLM
