...
console.log(
  "✅ Using Notion token:",
  process.env.NOTION_TOKEN.slice(0, 15) + "...",
);
...
function extractText(block: any): string {
  ...
  if (type === "file") {
    const caption =
      block.file?.caption?.map((t: any) => t.plain_text).join("") || "";
  ...
  if (type === "pdf") {
    const caption =
      block.pdf?.caption?.map((t: any) => t.plain_text).join("") || "";
  ...
  if (type === "audio") {
    const caption =
      block.audio?.caption?.map((t: any) => t.plain_text).join("") || "";
  ...
  switch (prop.type) {
    ...
    case "relation":
      return prop.relation?.length
        ? `${prop.relation.length} linked items`
        : null;
    case "rollup":
    ...
    try {
      ...
      for (const block of res.results) {
        ...
        if ("has_children" in block && block.has_children) {
          try {
            const childTexts = await fetchBlocksRecursively(
              block.id,
              depth + 1,
            );
            ...
            if (childError.code === "validation_error") {
              console.warn(
                `   ⚠️  Skipping unsupported child block: ${childError.message}`,
              );
            } else {
              throw childError;
            }
      ...
      if (
        ...
        error.message.includes("not supported via the API")
      ) {
        console.warn(
          `   ⚠️  Skipping unsupported block type: ${error.message}`,
        );
        break;
      } else {
        throw error;
    ...
    for (const item of res.results) {
      ...
      try {
        ...
        if ("created_by" in item) {
          const createdBy =
            (item.created_by as any)?.object === "user"
              ? (item.created_by as any).person?.name ||
                (item.created_by as any).name ||
                (item.created_by as any).id
              : "Unknown";
        ...
        if ("last_edited_by" in item) {
          const editedBy =
            (item.last_edited_by as any)?.object === "user"
              ? (item.last_edited_by as any).person?.name ||
                (item.last_edited_by as any).name ||
                (item.last_edited_by as any).id
              : "Unknown";
